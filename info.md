...
| Tool | 讀什麼 | 用途 |
|------|--------|------|
| `recall_history` | `<rag persist_dir>/chat_history/` ChromaDB | 語義搜尋已成功持久化、且不在目前 prompt 裡的舊對話內容；可選 `role` filter（`user` / `assistant`）。每筆結果帶 `role`、`turn_id`、`timestamp` |

已持久化的舊對話跨 process 可查 — 關掉 CLI、重新啟動後，agent 仍可搜到已寫入 `chat_history` 的部分。active session 期間，最近 `agent_recent_turns_window` 輪仍優先放在 prompt 裡，不會每輪自動塞回 prompt；CLI 正常結束時會把剩下的 recent turns flush 到 Chroma，讓短對話下次啟動也能被 `recall_history` 查到。**不是 rag_search 的替代品**：問題如果是關於知識庫文件，依然走 rag_search。
...
每完成一輪後 `ChatSession._evict_overflow` 檢查 `recent_turns`：超過 window 就把最舊那輪交給 `agent/history_rag/` 的 `ChatHistoryStore`，把 user prompt 跟 assistant response 各自 embed 成一個 chunk 寫入 `<rag persist_dir>/chat_history/`（單一 collection，metadata 帶 `role` / `turn_id` / `session_id` / `timestamp`）。寫入後才從 `recent_turns` 移除；成功寫入前，prompt pruning 不會把這些尚未持久化的 turn 丟掉。CLI 收到 `q` / EOF / Ctrl-C 結束時，會再呼叫 `flush_recent_turns()`，把剩下仍在 prompt window 裡的 completed turns 也寫入 Chroma。

**沒有 LLM 壓縮成本**：embedding 用本地 Ollama bge-m3，只在 eviction 或 session shutdown flush 時跑 `add_documents`，沒有 OpenRouter 呼叫。**內容不失真**：原文逐字保留在 ChromaDB，需要時透過 `recall_history` 取回，agent 看到的是當時實際說過的話。**跨 session 累積**：同一個 persist dir 永遠只有一個 collection，所有已持久化的舊對話都在裡面；active session 的最近 window 輪仍留在 prompt，直到被 eviction 或 CLI 結束時 flush。
...
- CLI shutdown flush 失敗 → `logger.warning`，該輪留在 `recent_turns`；但 process 結束後 RAM 仍會消失，所以這是需要修 Ollama/Chroma 環境的明示失敗。
...
- `await session.flush_recent_turns()` — CLI 結束前呼叫，把尚未被 window eviction 的 completed turns 寫入 `chat_history`
