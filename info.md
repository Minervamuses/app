...
### 長期記憶（turn-aware rolling compaction）

對話歷史不是無限塞進 prompt。`agent/memory.py` 定義一個三層結構：

1. **固定 system prompt**
2. **rolling summary**（單一 SystemMessage，整段壓縮後的「舊記憶」）
3. **recent turns**（最近幾輪還沒被壓縮的原始 user/assistant 訊息）

每滿 `config.agent_turns_per_compaction` 輪（預設 10），最舊那一整塊被 LLM 壓縮進 rolling summary 並從 recent_turns 移除。**tool 的原始 payload 永遠不會被帶入長期記憶** — 只留「這輪發生了什麼、結論是什麼」。

壓縮本身呼叫的是 `agent_compaction_model`（沒設就用 `llm_model`），跟主對話 LLM 共用同一個 `OpenRouterLLM`。
...
啟動參數：
- `--no-mcp` — 不載入任何 MCP server，只綁三個本地 KB tool
