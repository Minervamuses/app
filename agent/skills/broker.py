...
def resolve_capabilities(
    ...
    Deny rules win over capability grants. Unknown capabilities are ignored so
    manifests remain portable across runtimes with different tool surfaces.
    """
    if not manifest:
        return set(), set()
    ...
    allowed: set[str] = set()
    for capability_id in _requested_capability_ids(manifest):
        allowed.update(_tools_for_capability(
            capability_id,
...
def _requested_capability_ids(manifest: Mapping[str, Any]) -> list[str]:
